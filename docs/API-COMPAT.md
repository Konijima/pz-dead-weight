...
| `SandboxVars.DeadWeight.WeighCarried` (boolean, default true) | PROVEN (client decompiled `CustomSandboxOptions` registers mod options into the sandbox table; `GameClient.receiveSandboxOptions` carries them to MP clients) | UNPROVEN | read as `type(SandboxVars) == "table"` then `SandboxVars.DeadWeight.WeighCarried == true`; nil anywhere means off (`Occupants.weighCarried`) |
...
| `IsoGameCharacter:getForwardDirection()` (is the viewer facing the scale) | PROVEN by decompiled source (returns a `Vector2` in world x,y, the direction the character faces) | UNPROVEN | guarded and in a `pcall` in `Detect.facing`; a missing API or an error counts as facing. Within 0.6 squares of the scale centre it always counts (`Detect.faceCos` 0.35 otherwise) |
| `IsoObject:getRenderYOffset()` (height a scale sits above the floor, on a counter) | PROVEN by decompiled source (`ISMoveableSpriteProps` calls `setRenderYOffset(currentSurface)` when placing on a surface; 1x pixels, 96 to a tile) | UNPROVEN | `Occupants.baseZ` reads it as `offset / 96`, missing means 0; `Menu.findScaleSquare` hides "Step on Scale" when it is above 0 |
| `common/media/tileGeometry.txt` (`VERSION = 2`) and `common/media/depthmaps/DEPTH_<tileset>.png` | PROVEN by decompiled source (mod depth maps load only when `tileGeometry.txt` exists; pixel alpha 0 is not drawn, else blue/255 is the depth, non zero) and confirmed in game (2026-09-21): the Digital Scale draws over a counter and under a standing survivor | UNPROVEN (Build 42 only) | none: without the map the scale ties with the counter top and disappears; written by `tools/gen-tiles.py` |
| `Events.OnPreDistributionMerge` + `ProceduralDistributions.list.<List>.items` (loot: `BathroomCounter`, `BathroomCounterEmpty/Motel/NoMeds`, counters only) | PROVEN by the game's own Lua (`media/lua/server/Items/SuburbsDistributions.lua` hooks the same event; the lists exist in `ProceduralDistributions.lua`, items are `"Name", weight` pairs, `Mov_*` moveables are already listed there) | UNPROVEN (Build 42 only, the item is B42) | every list is `type(...) == "table"` checked, a missing one is skipped; the weight is `SandboxVars.DeadWeight.HomeScaleSpawn` or 3 when it is not readable at merge time (whether the sandbox table is loaded that early is UNPROVEN) |
| `MapObjects.OnNewWithSprite(sprite, fn, priority)` (once per square of a chunk generated for the first time, `IsoChunk.addZombies` gate) and `Events.LoadChunk` | PROVEN by decompiled source (`MapObjects.newGridSquare` runs only when the chunk is new, `IsoChunk` triggers `LoadChunk` after its squares are built; vanilla `MORandomPottedPlant.lua` uses `OnNewWithSprite`). Used INSTEAD of `RoomDef.explored`, which the game sets for a whole building the moment it is spotted (`IsoRoom.onSee`, `IsoWorld` on spawn) | UNPROVEN | registered on 20 toilet sprites, only when not `isClient()`; the hook only queues the square, the room is examined at `LoadChunk` |
| `IsoRoom:getName()/getSquares()/getRoomDef():getID()`, `IsoGridSquare:isWallTo/getDoorTo/getDoorFrameTo/getWindowTo/getWindowFrameTo/getAdjacentSquare/isFree/AddSpecialObject/RecalcProperties`, `IsoObject.new(cell, square, sprite)` (a scale placed on a bathroom floor) | PROVEN by decompiled source and by `ISMoveableSpriteProps.lua` (same `IsoObject.new` + `AddSpecialObject` + `RecalcProperties`) | UNPROVEN | no transmit: the chunk is new, so it reaches clients only after this runs; whether a dedicated server hands the added object to clients is UNPROVEN |
